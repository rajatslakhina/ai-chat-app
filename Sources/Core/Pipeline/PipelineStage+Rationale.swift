// The extended rationale for why each pipeline stage sits where it does.
//
// Split out of `PipelineStage.swift` on 2026-09-15 (second split; the first moved `package`
// and `title` into `PipelineStage+Catalog.swift`). Cases cannot themselves move — every case
// of an enum must live in the enum's own body — so the doc-comment prose above them is the
// only lever left. A pure text move: nothing below is compiled or reachable at runtime.
// `PipelineStage.swift` keeps a one-line pointer per stage; the reasoning it points to is
// here, keyed by case name.
