...
| Unit + integration tests | **1039 passing in 163 suites**, 0 failing. `LiveOpenRouterTests` return early without a key rather than being reported as skipped, so they are inside that count as passes that asserted nothing. |
...
the stage, which is the habit the drift argues for, and to 71 on 2026-09-18 the same way.) See [Coverage](#coverage).
