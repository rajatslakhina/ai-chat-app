...
| Unit + integration tests | **1013 passing in 161 suites**, 0 failing. `LiveOpenRouterTests` return early without a key rather than being reported as skipped, so they are inside that count as passes that asserted nothing. |
...
which is a good argument for the test.) See [Coverage](#coverage).
