...
| Unit + integration tests | **1031 passing in 162 suites**, 0 failing. `LiveOpenRouterTests` return early without a key rather than being reported as skipped, so they are inside that count as passes that asserted nothing. |
...
the stage, which is the habit the drift argues for.) See [Coverage](#coverage).
